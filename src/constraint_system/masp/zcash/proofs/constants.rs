...
use crate::constraint_system::masp::zcash::primitives::constants::{PEDERSEN_HASH_CHUNKS_PER_GENERATOR, PEDERSEN_HASH_GENERATORS};
use crate::constraint_system::masp;
...
pub const MONTGOMERY_A: BlsScalar = BlsScalar::from_raw([
...
/// The number of chunks needed to represent a full BlsScalar during fixed-base
/// exponentiation.
...
pub type FixedGenerator = &'static [Vec<(BlsScalar, BlsScalar)>];

/// Circuit version of a generator for fixed-base salar multiplication.
pub type FixedGeneratorOwned = Vec<Vec<(BlsScalar, BlsScalar)>>;


lazy_static! {
    pub static ref PROOF_GENERATION_KEY_GENERATOR: FixedGeneratorOwned =
        generate_circuit_generator(masp::zcash::primitives::constants::PROOF_GENERATION_KEY_GENERATOR);

    pub static ref NOTE_COMMITMENT_RANDOMNESS_GENERATOR: FixedGeneratorOwned =
        generate_circuit_generator(masp::zcash::primitives::constants::NOTE_COMMITMENT_RANDOMNESS_GENERATOR);

    pub static ref NULLIFIER_POSITION_GENERATOR: FixedGeneratorOwned =
        generate_circuit_generator(masp::zcash::primitives::constants::NULLIFIER_POSITION_GENERATOR);

    pub static ref VALUE_COMMITMENT_VALUE_GENERATOR: FixedGeneratorOwned =
        generate_circuit_generator(masp::zcash::primitives::constants::VALUE_COMMITMENT_VALUE_GENERATOR);

    pub static ref VALUE_COMMITMENT_RANDOMNESS_GENERATOR: FixedGeneratorOwned =
        generate_circuit_generator(masp::zcash::primitives::constants::VALUE_COMMITMENT_RANDOMNESS_GENERATOR);

    pub static ref SPENDING_KEY_GENERATOR: FixedGeneratorOwned =
        generate_circuit_generator(masp::zcash::primitives::constants::SPENDING_KEY_GENERATOR);
...
fn generate_circuit_generator(gen_aff: JubJubAffine) -> FixedGeneratorOwned {
    ...
    for _ in 0..FIXED_BASE_CHUNKS_PER_GENERATOR {
        ...
        let mut g = gen.clone();
        for _ in 0..7 {
            let g_affine = JubJubAffine::from(g);
            coeffs.push((g_affine.get_x(), g_affine.get_y()));
            g += gen;
        }
        windows.push(coeffs);
